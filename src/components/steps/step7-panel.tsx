...
export default function Step7Panel() {
  ...
  const handleExport = async () => {
    ...
    setError(null);
    try {
      const scaleFactor = state.step6Data?.validation?.bestScaleFactor || 1;
          ...
          {state.step6Data && (
            <div className="bg-white rounded-lg border border-slate-200 p-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="p-3 bg-amber-50 rounded">
                  <div className="text-xs text-slate-500 mb-1">缩放因子</div>
                  <div className="font-mono font-semibold text-amber-700">
                    {state.step6Data.validation.bestScaleFactor.toFixed(4)}
                  </div>
                </div>
                <div className="p-3 bg-emerald-50 rounded">
